...
describe('EncoderAudioWorkletProcessor', () => {
  beforeAll(async () => {
    ...
    if (registerProcessorMock.mock.calls.length > 0) {
      ...
      throw new Error("Processor not registered in beforeAll. Check module import or mocking.");
    }
  });
  ...
  it('should be registered with the name "encoder-audio-worklet" once', () => {
    ...
    expect(registerProcessorMock).toHaveBeenCalledWith('encoder-audio-worklet', EncoderAudioWorkletProcessor);
  });

  describe('constructor', () => {
    it('should set up port.onmessage', () => {
      if (!EncoderAudioWorkletProcessor) throw new Error('Processor not defined');
  ...
  describe('process', () => {
    ...
    beforeEach(() => {
      ...
      mockWorkerSidePort = { postMessage: vi.fn() as Mock<(...args: any[]) => any> };
      ...
      if (processor.port && typeof processor.port.onmessage === 'function') {
        ...
        throw new Error("Processor port or onmessage handler not set up correctly for process tests.");
      }
    });
    ...
    it('should post audio data to workerPort if inputs are valid', () => {
      const inputData = [new Float32Array([0.1, 0.2, 0.3]), new Float32Array([0.4, 0.5, 0.6])];
      ...
      const actualAudioData = mockWorkerSidePort.postMessage.mock.calls[0][0].audioData;
