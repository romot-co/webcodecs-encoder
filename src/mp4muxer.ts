...
import type { EncoderConfig, MainThreadMessage, WorkerDataChunkMessage } from './types';
import { EncoderErrorType } from './types';

// MuxerOptions がエクスポートされていないため、ConstructorParameters を使用して型を推論し、
// fastStart プロパティを明示的に追加します。
type BaseOptions = ConstructorParameters<typeof Muxer<ArrayBufferTarget | StreamTarget>>[0];
...
interface ExtendedMuxerOptions extends Omit<BaseOptions, 'target' | 'video' | 'audio'> {
...
export class Mp4MuxerWrapper {
  ...
  private postMessageToMain: (message: MainThreadMessage, transfer?: Transferable[]) => void;
  private config: EncoderConfig;

  constructor(
    config: EncoderConfig,
    postMessageCallback: (message: MainThreadMessage, transfer?: Transferable[]) => void
  ) {
    this.config = config;
    ...
    const muxerAudioCodec = audioCodecOption as 'aac' | 'opus';
  ...
  addVideoChunk(chunk: EncodedVideoChunk, meta?: EncodedVideoChunkMetadata): void {
    if (!this.videoConfigured) {
      this.postMessageToMain({
        type: 'error',
        errorDetail: { message: 'MP4: Video track not configured.', type: EncoderErrorType.ConfigurationError }
      });
      return;
    }
    try {
      ...
      this.postMessageToMain({
        type: 'error',
        errorDetail: { message: `MP4: Error adding video chunk: ${e.message}`, type: EncoderErrorType.MuxingFailed, stack: e.stack }
      });
    }
  }
  ...
  addAudioChunk(chunk: EncodedAudioChunk, meta?: EncodedAudioChunkMetadata): void {
    if (!this.audioConfigured) {
      this.postMessageToMain({
        type: 'error',
        errorDetail: { message: 'MP4: Audio track not configured.', type: EncoderErrorType.ConfigurationError }
      });
      return;
    }
     try {
      ...
      this.postMessageToMain({
        type: 'error',
        errorDetail: { message: `MP4: Error adding audio chunk: ${e.message}`, type: EncoderErrorType.MuxingFailed, stack: e.stack }
      });
    }
  }
  ...
  finalize(): Uint8Array | null {
    if (this.config.latencyMode === 'realtime') {
      ...
      try {
        ...
        this.postMessageToMain({
          type: 'error',
          errorDetail: { message: `MP4: Error finalizing muxer (realtime): ${e.message}`, type: EncoderErrorType.MuxingFailed, stack: e.stack }
        });
      }
    ...
    if (!(this.target instanceof ArrayBufferTarget)) {
        this.postMessageToMain({
            type: 'error',
            errorDetail: { message: 'MP4: Muxer target is not ArrayBufferTarget in non-realtime mode.', type: EncoderErrorType.InternalError }
        });
        return null;
    }
    ...
    try {
      ...
      return new Uint8Array(buffer);
       ...
       this.postMessageToMain({
        type: 'error',
        errorDetail: { message: `MP4: Error finalizing muxer (non-realtime): ${e.message}`, type: EncoderErrorType.MuxingFailed, stack: e.stack }
      });
      return null;
    }
  }
