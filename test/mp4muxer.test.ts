...
vi.mock("mp4-muxer", () => {
  ...
  let capturedStreamTargetOnData:
    | ((chunk: Uint8Array, position: number) => void)
    | null = null;
  ...
  return {
    ...
    _clearLastMuxerTargetInstance: () => {
      lastMuxerTargetInstance = null;
    },
  };
});
...
interface MockedMp4Muxer {
  ...
  _getCapturedStreamTargetOnData: () =>
    | ((chunk: Uint8Array, position: number) => void)
    | null;
...
describe("Mp4MuxerWrapper", () => {
  ...
  beforeEach(async () => {
    ...
    resetCapturedStreamTargetOnData =
      mockedModule._resetCapturedStreamTargetOnData;
  ...
  it("adds video chunks", () => {
    ...
    const meta = {
      decoderConfig: { codec: "avc1.42001f", description: new Uint8Array(5) },
  ...
  describe("constructor video codec options", () => {
    ...
    it("should use 'av1' for video codec when specified", () => {
      const av1Config: EncoderConfig = {
        ...baseConfig,
        codec: { video: "av1", audio: "aac" },
      };
    ...
    it("should default to 'avc' for unknown or unspecified video codec", () => {
      const unknownCodecConfig = {
        ...baseConfig,
        codec: { video: "unknown" as any, audio: "aac" as const },
      }; // Use 'as any' for video, 'as const' for audio
      ...
      const unspecifiedCodecConfig: EncoderConfig = {
        ...baseConfig,
        codec: { audio: "aac" },
      }; // video is implicitly undefined
  ...
  describe("finalize error handling and realtime", () => {
    ...
    it("should post error if target is not ArrayBufferTarget in non-realtime finalize", async () => {
      ...
      (wrapper as any).target = new StreamTarget(
        (_data: Uint8Array, _position: number) => {},
      );
      wrapper.finalize();
      expect(postMessageCallback).toHaveBeenCalledWith(
        expect.objectContaining({
